...
version = 'v20240514'
type = 'zone' # Choose 'zone', 'cavm_fine', 'cavm_coarse', 'compare_raynolds', 'compare_spawn'
...
biomass = biomass %>% dplyr::select(-one_of(c('system.index', '.geo', 'woody_dominance_sum')))
...
if(!type %in% c('compare_raynolds', 'compare_spawn')){
...
if(type == 'zone'){
  ...
  biomass = biomass %>%
    dplyr::select(-dsl) %>% 
    pivot_longer(!c(FIRST_zone, woody_dominance_mean, woody_percent), names_to = "type", values_to = "biomass_g")
  ...
  # Pivot longer
  biomass = biomass %>%
    pivot_longer(!c(veg_code, veg_description, woody_dominance_mean, woody_percent, veg_category), names_to = "type", values_to = "biomass_g")
  ...
  # Pivot longer
  biomass = biomass %>%
    pivot_longer(!c(veg_code, woody_dominance_mean, woody_percent, veg_category), names_to = "type", values_to = "biomass_g")
...
biomass$biomass_type = plyr::revalue(biomass$biomass_type, c("total" = "Plant", "nonwoody" = "Nonwoody Plant", "woody" = "Woody Plant", "bgb" = "Belowground"))
