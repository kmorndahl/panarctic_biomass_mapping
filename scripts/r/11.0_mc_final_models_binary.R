...
# This script runs final probability model Monte Carlo iterations (total and woody):
...
selection_metric = 'roc_auc' # Metric for choosing best parameters
binary_threshold_metric = 'f_meas' # Metric used to choose best threshold
...
out_dir = 'output/11_mc_final_models/binary/'
...
# Spectral summaries - we need to recalculate these using permuted data
...
# ==============================================================================
# 11. SET UP BLOCKED RESPONSE VARIABLE =========================================
# ==============================================================================
...
predictions = predict(final_wf, biomass_df, type = "prob")

# Aggregate training set predictions
predictions_df =  st_drop_geometry(biomass_df)
predictions_df$prob_presence = data.frame(predictions)$.pred_presence
predictions_df$prob_absence = data.frame(predictions)$.pred_absence
...
final_rec = recipe(formula = update(form,    ~ . + site_code), data = biomass_df) %>%
  ...
  step_select_mrmr(all_predictors(),
                   outcome = 'presence_absence',
                   threshold = final_params$feat_thresh / 100) %>% 
  step_novel(all_factor_predictors(), new_level = 'other') %>% # Allow recipe to identify factor levels not seen in the training data and treat them as a new level called "other"
  step_dummy(all_factor_predictors(), one_hot = TRUE) # One hot encode to keep all factor levels for variable importance purposes
...
pred_fun = function(object, newdata) {
  predict(object, newdata, type = "response")$predictions[,1] # first column = presence, second column = absence
}
...
write.csv(final_predictors, paste0(out_dir, '/predictors/final_predictors_', ds_type, '_binary_', out_version, '_', i,  '.csv'), row.names = FALSE)
write.csv(predictions_df, paste0(out_dir, '/predictions/final_predictions_', ds_type, '_binary_', out_version, '_', i, '.csv'), row.names = FALSE)
write.csv(final_params, paste0(out_dir, '/parameters/final_parameters_', ds_type, '_binary_', out_version, '_', i,  '.csv'), row.names = FALSE)
saveRDS(engine_mod, paste0(out_dir, '/models/', ds_type, '_binary_', out_version, '_', i,  '.rds'))
write.csv(final_data_juiced, paste0(out_dir, '/final_dat/final_data_', ds_type, '_binary_', out_version, '_', i,  '.csv'), row.names = FALSE)
write.csv(vi_perm, paste0(out_dir, '/vip/vip_permutation_', ds_type,  '_binary_', out_version, '_', i,  '.csv'), row.names = FALSE)
write.csv(vi_shap, paste0(out_dir, '/vip/vip_shap_', ds_type,  '_binary_', out_version, '_', i,  '.csv'), row.names = FALSE)
