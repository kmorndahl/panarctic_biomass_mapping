...
DESCRIPTION: Calculate masked area components (Greenland ice, inland water, other/Landsat gaps)
and create combined water/snow/ice mask
...
final_mask = ee.Image('projects/arctic-biomass-mapping/assets/modeled_final/total_biomass_2020_j_index_v20240514').select('total_biomass').mask()
arctic_buffer = ee.FeatureCollection('projects/arctic-biomass-mapping/assets/ROIs/arctic_oroarctic_coast_buffer_laea_final')
...
# ======================================
# 2. CREATE MASKS AND CALCULATE AREA ===
# ======================================
...
# Greenland ice data
greenland_ice = ee.Image("OSU/GIMP/2000_ICE_OCEAN_MASK").select('ice_mask').rename('water_mask')

# Tidy Greenland data
greenland_ice = greenland_ice.eq(1).unmask(value=0, sameFootprint=False).selfMask().rename('mask')

# Calculate area
greenland_ice_area = area.updateMask(greenland_ice.mask())
greenland_ice_area_summary = greenland_ice_area.reduceRegions(
    reducer=ee.Reducer.sum(),
    collection=ee.FeatureCollection([ee.Feature(ee.Geometry.Polygon(params.export_region_coords, None, False), None)]),
    scale=scale,
    crs=crs
)
...
inland_water_mask = ee.Image(ee.Algorithms.If(
    ee.Number(buffer_dist).eq(0),
    water_mask,
...
# Calculate area
inland_water_area = area.updateMask(inland_water_mask.mask())
inland_water_area_summary = inland_water_area.reduceRegions(
    reducer=ee.Reducer.sum(),
    collection=ee.FeatureCollection([ee.Feature(ee.Geometry.Polygon(params.export_region_coords, None, False), None)]),
    scale=scale,
    crs=crs
...
landsat_mask = (final_mask.add(inland_water_mask.select('mask').unmask(0))
...
# Calculate area
other_area = area.updateMask(landsat_mask.mask())
other_area_summary = other_area.reduceRegions(
    reducer=ee.Reducer.sum(),
    collection=ee.FeatureCollection([ee.Feature(ee.Geometry.Polygon(params.export_region_coords, None, False), None)]),
    scale=scale,
    crs=crs
...
water_snow_ice = greenland_ice.unmask(0).add(inland_water_mask.unmask(0)).add(landsat_mask.unmask(0)).selfMask()
