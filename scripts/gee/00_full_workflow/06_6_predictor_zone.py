"""

DESCRIPTION: Create zone predictor

AUTHOR: Kathleen Orndahl
DATE: 11-10-2024

NOTES:

TO-DO:

"""

import ee
from utils import params

try:
    ee.Initialize(project=params.ee_project)
except Exception:
    ee.Authenticate()
    ee.Initialize(project=params.ee_project)

# =========================
# 1. SET-UP ===============
# =========================

# 1.0 ----- PARAMETERS -----

# Reprojection options
crs = params.crs
scale = params.scale

# 1.1 ----- READ IN DATA -----

zones = ee.FeatureCollection("projects/arctic-biomass-mapping/assets/ROIs/zones_with_subarctic")

# =========================
# 2. ANALYSIS =============
# =========================

# Tidy zones
def tidy_zone_name(feat):
    return feat.set({'FIRST_zone': ee.String(feat.get('FIRST_zone')).replace(' ', '', 'g')})

zones = zones.map(tidy_zone_name)

# Convert zones to image
zones_img = zones.reduceToImage(
    properties=['dsl'],
    reducer=ee.Reducer.first()
)

# Add 'other' category
zones_img = zones_img.unmask(5)  # All masked areas are converted to 5 to represent 'zone_3_other'

# Convert single band zone image to multi-band binary zone image
zone_nums = ee.List.sequence(1, 5, 1)

def make_zone_band(num):
    return zones_img.eq(ee.Image.constant(num)).unmask(0)

zones_final = ee.ImageCollection(zone_nums.map(make_zone_band))
zones_final = (zones_final
    .toBands()
    .rename(['zone_3_HighArctic', 'zone_3_LowArctic', 'zone_3_OroArctic', 'zone_3_SubArctic', 'zone_3_other'])  # Remove prefixes added by 'toBands()'
    .setDefaultProjection(crs=crs, scale=scale))

# Report
print('Bioclimate Zones', zones_final.getInfo())

# =========================
# 3. EXPORT ===============
# =========================

task = ee.batch.Export.image.toAsset(
    image=zones_final,
    description='zones_img',
    assetId='projects/arctic-biomass-mapping/assets/predictors/zones_img',
    region=ee.Geometry.Polygon(params.export_region_coords, None, False),  # Specify as rectangle to avoid gaps in output
    scale=scale,
    crs=crs,
    maxPixels=1e13
)
task.start()

raise Exception('stop')