...
DESCRIPTION: Run Monte Carlo probability forests (fit in R) across Pan Arctic domain
...
asset_path = 'projects/arctic-biomass-mapping/assets/modeled_binary/model_binary_' + str(year) + '_' + ds_type + '_' + model_version + '_mc/'  # Asset path for export
mc_list = ee.List.sequence(100, 100, 1).getInfo()
...
def run_mc_iteration(mc):
    ...
    mod_str = ee.List([
        ee.Blob(
            'gs://arctic_biomass_mapping_models/' + model_version + '/mc/binary/final_trees/' + ds_type + '_binary_formatted_gee_short_' + model_version + '_' + str(int(mc)) + '.txt'
        ).string()
    ])
    ...
    mod = ee.Classifier.decisionTreeEnsemble(mod_str).setOutputMode('RAW')
    # print('Model:', mod.getInfo())

    # Get information about the trained classifier
    # print('Results of trained classifier', mod.explain().getInfo())

    # 2.1 ----- EXISTING DATA PATHS -----

    # CCDC
    phenology = ee.ImageCollection(
        'projects/arctic-biomass-mapping/assets/seasonal_percentile_doys/seasonal_doys_tiles_' + str(start_year) + '_' + str(end_year) + '_' + ccdc_version
    ).mosaic()
    refl = ee.Image(
    ...
    # Model information
    selected_predictors = ee.FeatureCollection(
        "projects/arctic-biomass-mapping/assets/model_ready_data/" + model_version + "/mc/binary/final_predictors/final_predictors_" + ds_type + "_binary_" + model_version + '_' + str(int(mc))
    )
    ...
    selected_predictors = ee.List(
        selected_predictors.aggregate_array('predictor').map(
            lambda item: ee.String(item).replace('[.]', '', 'g')
        )
    )
    predictor_categories = selected_predictors.map(
        lambda predictor_name: ee.String(predictor_name).replace('_.*$', '')
    )
    # print('Selected predictors: ', selected_predictors.getInfo())
    # print('Predictor categories: ', predictor_categories.getInfo())
    ...
    texture = ee.Image(
        "projects/arctic-biomass-mapping/assets/predictors/seasonal_modeled_reflectance_NDVI_texture_" + str(year) + "_" + ccdc_version
    )
    # print('Texture:', texture.getInfo())
    ...
    wte_proj = wte.select('world_terrestrial_ecosystems_X112').geometry().projection()
    wte_crs = wte.select('world_terrestrial_ecosystems_X112').geometry().projection().crs()
    wte_transform = wte.select('world_terrestrial_ecosystems_X112').geometry().projection().getInfo()['transform']
    ...
    # print('Zones:', zones_final.getInfo())
    ...
    # print('Spectral:', spectral.getInfo())
    ...
    predictors = ee.Image(
        ee.Algorithms.If(
            predictor_categories.contains('topo'),
            predictors.addBands(topographic),
            predictors
        )
    )

    # print('Final predictors: ', predictors.getInfo())
    ...
    # Apply model
    results = predictors.classify(mod, 'raw_probability')

    # Get probability of presence
    prob_presence = (results.arrayReduce(ee.Reducer.mean(), ee.List([0]))
                            .arrayGet(0)
                            .rename('prob_presence'))
    ...
    task = ee.batch.Export.image.toAsset(
        image=prob_presence.multiply(100).toInt8(),  # Scale for data storage
        description='model_binary_' + str(year) + '_' + ds_type + '_' + model_version + '_' + str(int(mc)),
        assetId=asset_path + str(int(mc)),
