...
# Copernicus slope, 30 m resolution
slp = ee.ImageCollection('projects/arctic-biomass-mapping/assets/predictors/copernicus_30m_slope_gcs').mosaic()
...
for tile_name in tile_list_js: # LOOP TILES
    ...
    for season in seasons: #  LOOP SEASONS
        ...
        num_pixels = refl_ic.select('spectral_blue_' + season).reduceRegion(
            reducer = ee.Reducer.count(), 
            geometry = tile.geometry(), 
            crs = crs, 
            scale = scale, 
            maxPixels = 1e13, 
            tileScale = tile_scale) # Adjust tileScale as necessary to account for out of memory errors and computation timed out errors
        num_pixels = ee.Number(num_pixels.values().get(0)).getInfo()
        ...
        for band in season_bands: # LOOP BANDS
            ...
            random_pixels = refl_ic.select(ee.List(['IC_' + season, band])).addBands(refl_ic.select(band).mask().rename('class').byte()).stratifiedSample(numPoints = pixel_count.multiply(frac).int(), classBand = 'class', region = tile.geometry(), crs = crs, scale = scale, seed = 42, dropNulls = True, geometries = False, tileScale = tile_scale); # Adjust tileScale as necessary to account for out of memory errors and computation timed out errors
        ...
        if num_pixels < 100:
            refl_append = refl.select(season_bands)
        else:
            refl_append = refl_SCSccorr
